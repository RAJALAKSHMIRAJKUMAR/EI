...
BEGIN
	...
	MAIN_LOOP : LOOP
		...
		IF(EEUNITNO IS NOT NULL AND EEINVOICEDATE IS NOT NULL AND EEFROMPERIOD IS NOT NULL AND EETOPERIOD IS NOT NULL AND EEECNID IS NOT NULL AND EEAMOUNT IS NOT NULL AND USERSTAMP IS NOT NULL)THEN 
			CALL SP_BIZDLY_SUBELECTRICITY_INSERT(EEUNITNO,EEINVOICEDATE,EEFROMPERIOD,EETOPERIOD,EEECNID,EEAMOUNT,EECOMMENTS,USERSTAMP,@EESUCCESSFLAG);
		...
		IF(SUBSPSUCCESSFLAG=0)THEN
			...
			'COMMENTS=',EECOMMENTS,',','USERSTAMP=',USERSTAMP));
		END IF;
