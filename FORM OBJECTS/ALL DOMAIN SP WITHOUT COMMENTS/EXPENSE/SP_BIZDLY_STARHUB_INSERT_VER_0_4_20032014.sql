...
MAIN_LOOP : LOOP
	...
	IF(ESUNITNO IS NOT NULL AND ESINVOICEID IS NOT NULL AND ESINVOICEDATE IS NOT NULL AND ESFROMPERIOD IS NOT NULL AND ESTOPERIOD IS NOT NULL AND ESAMOUNT IS NOT NULL AND USERSTAMP IS NOT NULL)THEN 
		CALL SP_BIZDLY_SUBSTARHUB_INSERT(ESUNITNO,ESINVOICEID,ESINVOICEDATE,ESFROMPERIOD,ESTOPERIOD,ESAMOUNT,ESCOMMENTS,USERSTAMP,@STARHUB_SUCCESSFLAG,@STARHUB_UPDATEFLAG);
	...
	IF(SUBSPSUCCESSFLAG=0)THEN
		SET FINALMESSAGE = (SELECT CONCAT(FINALMESSAGE,' ','UNIT_NO=',ESUNITNO,',INVOICETO=',ESINVOICECHECK,',INVOICEDATE=',ESINVOICEDATE,',FROMPERIOD=',ESFROMPERIOD,',TOPERIOD=',ESTOPERIOD,',AMOUNT=',ESAMOUNT,',COMMENTS=',ESCOMMENTS,',USERSTAMP=',USERSTAMP));
	END IF;
