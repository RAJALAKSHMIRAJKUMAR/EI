DROP PROCEDURE IF EXISTS SP_TEMP_PAYMENT_DETAILS_UPDATE_RECORD_VERSION_SPLIT5;
CREATE PROCEDURE SP_TEMP_PAYMENT_DETAILS_UPDATE_RECORD_VERSION_SPLIT5(IN SOURCESCHEMA  VARCHAR(40),IN DESTINATIONSCHEMA  VARCHAR(40))
BEGIN
	...
	SET @MINPDID = (SELECT CONCAT('SELECT MIN(PD_ID) INTO @MIN_PDID FROM ',DESTINATIONSCHEMA,'.TEMP_PAYMENT_DETAILS_SPLIT5'));
	PREPARE MINPDID_STMT FROM @MINPDID;
    EXECUTE MINPDID_STMT;
	SET MIN_PDID=@MIN_PDID;
	SET @MAXPDID = (SELECT CONCAT('SELECT MAX(PD_ID) INTO @MAX_PDID FROM ',DESTINATIONSCHEMA,'.TEMP_PAYMENT_DETAILS_SPLIT5'));
	PREPARE MAXPDID_STMT FROM @MAXPDID;
    EXECUTE MAXPDID_STMT;
	SET MAX_PDID=@MAX_PDID;
	WHILE(MIN_PDID<=MAX_PDID)DO
	    SET @FORPERIOD=NULL;
		SET @SELECT_FORPERIOD = (SELECT CONCAT('SELECT PD_FOR_PERIOD INTO @FORPERIOD FROM ',DESTINATIONSCHEMA,'.TEMP_PAYMENT_DETAILS_SPLIT5 WHERE PD_ID=',MIN_PDID));
		...
		SELECT TCED.CUSTOMER_ID,TCED.CED_REC_VER,TCED.CED_STARTDATE,TCED.CED_ENDDATE,TCED.CED_PRETERMINATE_DATE,TCED.CED_RECHECKIN FROM ',DESTINATIONSCHEMA,'.TEMP_CUSTOMER_ENTRY_DETAILS TCED WHERE TCED.CUSTOMER_ID=(SELECT CUSTOMER_ID FROM ',DESTINATIONSCHEMA,'.TEMP_PAYMENT_DETAILS_SPLIT5 WHERE PD_ID=',MIN_PDID,')AND
		TCED.UNIT_ID=(SELECT UNIT_ID FROM ',DESTINATIONSCHEMA,'.TEMP_PAYMENT_DETAILS_SPLIT5 WHERE PD_ID=',MIN_PDID,')
		...
		SET @TPDURV_PTDATE=NULL;
			...
			SET PREVIOUS_REC_ENDDATE_MONTH_YEAR = (SELECT CONCAT(PREVIOUS_REC_ENDDATE_MONTH,'-',PREVIOUS_REC_ENDDATE_YEAR));
				IF(CURRENT_SDATE_MONTH_YEAR=PREVIOUS_REC_ENDDATE_MONTH_YEAR)THEN
					...
					SET REC_VER=@REC_VER;
					SET @UPDATE_TEMP_PAYMENT_DETAILS=(SELECT CONCAT('UPDATE ',DESTINATIONSCHEMA,'.TEMP_PAYMENT_DETAILS_SPLIT5 SET CED_REC_VER=(',REC_VER,'-1) WHERE (PD_ID=(',MIN_PDID,'-1) AND PP_ID=1)'));
			...
			IF(FORPERIOD BETWEEN STARTDATE AND ENDDATE)THEN
				SET @UPDATE_REC_VER=(SELECT CONCAT('UPDATE ',DESTINATIONSCHEMA,'.TEMP_PAYMENT_DETAILS_SPLIT5 SET CED_REC_VER=(SELECT TPDURV_RECVER FROM ',DESTINATIONSCHEMA,'.TEMP_PAYMENT_DETAILS_UPDATE_RECORD_VERSION WHERE 
	...
	DROP TABLE IF EXISTS TEMP_PAYMENT_DETAILS_DURATION5;
	CREATE TABLE TEMP_PAYMENT_DETAILS_DURATION5(ID INTEGER NOT NULL AUTO_INCREMENT PRIMARY KEY,PAYMENT_DURATION5 TIME);
	INSERT INTO TEMP_PAYMENT_DETAILS_DURATION5(PAYMENT_DURATION5)VALUES(DURATION);
END;
