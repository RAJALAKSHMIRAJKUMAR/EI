-- version:0.9 -- sdate:20/05/2014 -- edate:24/05/2014 -- issue:803 --COMMENT NO: 75--desc:CHANGED CONDITIONS FOR DIFFERENT UNIT EXTENSION --doneby:DHIVYA
-- version:0.8 -- sdate:16/05/2014 -- edate:20/05/2014 -- issue:803 --COMMENT NO: 43--desc:CHANGED CONDITIONS FOR INSERTING DEPOSIT,PF AND DEPOSIT REFUND --doneby:DHIVYA
-- version:0.7 -- sdate:19/03/2014 -- edate:22/03/2014 -- issue:810 --COMMENT NO: 8--desc:CHANGED ALL CONCEPTS IN SP AND ALSO ADDED EXTENSION RECORD FOR PAYMENT AMOUNT AND RESTRICTED FUTURE CUSTOMER PAYMENT RECORDS --doneby:DHIVYA
                                ...
                                ELSE
                                    IF(PPID=1)OR(PPID=4) THEN
                                        ...
                                        IF(RECHECKINFLAG IS NOT NULL)THEN
                                                ...
                                                END IF;
                                                SET CLFEEID=(SELECT ID FROM TEMP_PAYMENT_ENTRY_DETAILS WHERE PD_FOR_PERIOD=FORPERIOD AND PP_ID=4 AND ID!=MINID AND CED_REC_VER=RECVER AND CUSTOMER_ID=MINCUSTOMERID);
                                                IF CLFEEID IS NOT NULL THEN
                                                    UPDATE TEMP_PAYMENT_ENTRY_DETAILS SET CED_REC_VER=(RECVER-1) WHERE ID=CLFEEID;
                                                END IF;
                                        END IF;
                                    END IF;
                                END IF;
