-- version:0.9 -- sdate:12/07/2014 -- edate:14/07/2014 -- issue:803 --COMMENT NO: 131--desc:CHANGED SOME CONDITIONS SAME MONTH CUSTOMER RECHECKIN AND EXTENDED IN DIFF UNIT --doneby:DHIVYA
...
DECLARE CLFEEID INTEGER;
DECLARE PAYMENTUNITID INTEGER;
DECLARE RE_RECVER INTEGER;
...
SELECT TPED.UNIT_ID,TPED.CUSTOMER_ID,TPED.PP_ID,TPED.CED_REC_VER,TPED.PD_AMOUNT,TPED.PD_FOR_PERIOD,TPED.PD_PAID_DATE,TPED.PD_HIGHLIGHT_FLAG,TPED.PD_COMMENTS,TPED.PD_USERSTAMP,TPED.PD_TIMESTAMP FROM ',DESTINATIONSCHEMA,'.TEMP_PAYMENT_DETAILS TPED,',DESTINATIONSCHEMA,'.TEMP_CUSTOMER_ENTRY_DETAILS TCED WHERE TPED.UNIT_ID=TCED.UNIT_ID AND TPED.CUSTOMER_ID=TCED.CUSTOMER_ID GROUP BY TPED.PD_ID ORDER BY TPED.CUSTOMER_ID,TCED.CED_REC_VER,TPED.PD_FOR_PERIOD '));
        ...
        WHILE MINID<=MAXID DO
            ...
            SET MAXPAYMENTID=(SELECT MAX(ID) FROM TEMP_PAYMENT_CUSTOMER);
                WHILE MINPAYMENTID<=MAXPAYMENTID DO
					...
					SET PAYMENTUNITID=(SELECT UNIT_ID FROM TEMP_PAYMENT_ENTRY_DETAILS WHERE ID=MINID);
                    ...
                    SET RECHECKINFLAG=(SELECT CED_RECHECKIN FROM TEMP_PAYMENT_CUSTOMER WHERE ID=MINPAYMENTID);
                        ...
                        IF FORPERIOD BETWEEN STARTDATE AND ENDDATE THEN
                            ...
                            SET NEXTUNITID=(SELECT UNIT_ID FROM TEMP_PAYMENT_CUSTOMER WHERE CUSTOMER_ID=MINCUSTOMERID AND CED_REC_VER=(RECVER-1));
                                IF (EXTENSIONFLAG IS NOT NULL) AND (PPID=1 OR PPID=4) AND (FORPERIOD=STARTDATE) AND ((PREUNITID=NEXTUNITID)) THEN
                                            ...
                                            IF LP_STARTDATE<LP_FUTURE_MONTH THEN
                                                IF PPID=1 THEN
                                                    IF PREUNITID!=NEXTUNITID THEN
														IF PAYMENTUNITID=PREUNITID THEN
                                                        SET UNITID=NEXTUNITID;
														END IF;
														IF PAYMENTUNITID=NEXTUNITID THEN
                                                        SET UNITID=PREUNITID;
														END IF;
                                                    ELSE
                                    ...
                                    ELSE
										...
										END IF;
                                    END IF;
                                    IF NOT EXISTS(SELECT PD_FOR_PERIOD FROM TEMP_PAYMENT_ENTRY_DETAILS WHERE CUSTOMER_ID=MINCUSTOMERID AND PD_FOR_PERIOD=FORPERIOD AND PP_ID=4 AND ID>MINID)THEN
                                        IF NOT EXISTS(SELECT CED_REC_VER FROM TEMP_PAYMENT_ENTRY_DETAILS WHERE CUSTOMER_ID=MINCUSTOMERID AND PD_FOR_PERIOD=FORPERIOD AND PP_ID=4 AND CED_REC_VER=RECVER)THEN
                                            IF LP_STARTDATE<LP_FUTURE_MONTH THEN
                                                IF PPID=4 THEN
                                                    IF PREUNITID!=NEXTUNITID THEN
														IF PAYMENTUNITID=PREUNITID THEN
                                        ...
                                        END IF;
                                    ELSE
                                        SET PAYMENTID=(SELECT MIN(ID) FROM TEMP_PAYMENT_ENTRY_DETAILS WHERE CUSTOMER_ID=MINCUSTOMERID AND PD_FOR_PERIOD=FORPERIOD AND PP_ID=4 AND ID>MINID AND CED_REC_VER IS NULL);
                                        IF PAYMENTID IS NOT NULL THEN
												UPDATE TEMP_PAYMENT_ENTRY_DETAILS SET CED_REC_VER=RECVER WHERE ID=PAYMENTID;
										END IF;
                                    END IF;
                                     ...
                                     END IF;
                                        SET CHECKRECVER=(SELECT CED_REC_VER FROM TEMP_PAYMENT_ENTRY_DETAILS WHERE ID=MINID);
                                            IF CHECKRECVER IS NULL THEN
                                                UPDATE TEMP_PAYMENT_ENTRY_DETAILS SET CED_REC_VER=RECVER WHERE ID=MINID;
                                            END IF;
											SET RE_RECVER=(SELECT MIN(CED_REC_VER) FROM TEMP_PAYMENT_ENTRY_DETAILS WHERE CUSTOMER_ID=MINCUSTOMERID AND PD_FOR_PERIOD=FORPERIOD AND CED_REC_VER=RECVER AND ID!=MINID AND PP_ID=1);
											IF RE_RECVER IS NOT NULL THEN
											UPDATE TEMP_PAYMENT_ENTRY_DETAILS SET CED_REC_VER=(SELECT MIN(CED_REC_VER) FROM TEMP_PAYMENT_CUSTOMER WHERE CUSTOMER_ID=MINCUSTOMERID AND UNIT_ID=PAYMENTUNITID AND CED_STARTDATE=FORPERIOD AND CED_REC_VER!=RE_RECVER)WHERE ID=MINID;
											ELSE
											SET RECVER=(SELECT MIN(CED_REC_VER) FROM TEMP_PAYMENT_CUSTOMER WHERE CUSTOMER_ID=MINCUSTOMERID AND UNIT_ID=PAYMENTUNITID AND FORPERIOD BETWEEN (SELECT ADDDATE(LAST_DAY(SUBDATE(CED_STARTDATE, INTERVAL 1 MONTH)), 1)) AND IF(CED_PRETERMINATE_DATE IS NOT NULL,CED_PRETERMINATE_DATE,CED_ENDDATE));
											UPDATE TEMP_PAYMENT_ENTRY_DETAILS SET CED_REC_VER=RECVER WHERE ID=MINID;
											END IF;
											IF CHECKRECVER IS NULL THEN
                                                UPDATE TEMP_PAYMENT_ENTRY_DETAILS SET CED_REC_VER=RECVER WHERE ID=MINID;
                    ...
                    SET FORPERIOD=(SELECT PD_FOR_PERIOD FROM TEMP_PAYMENT_ENTRY_DETAILS WHERE ID=MINID);
                    IF (PPID=2) THEN
                        SET UNITID=(SELECT UNIT_ID FROM TEMP_PAYMENT_ENTRY_DETAILS WHERE ID=MINID);
                        SET RECVER=(SELECT MIN(CED_REC_VER) FROM TEMP_PAYMENT_CUSTOMER WHERE CUSTOMER_ID=MINCUSTOMERID AND CED_STARTDATE=FORPERIOD AND UNIT_ID=UNITID);
                        IF NOT EXISTS(SELECT CED_REC_VER FROM TEMP_PAYMENT_ENTRY_DETAILS WHERE CUSTOMER_ID=MINCUSTOMERID AND CED_REC_VER=RECVER AND PP_ID=2 AND PD_FOR_PERIOD=FORPERIOD)THEN
                            UPDATE TEMP_PAYMENT_ENTRY_DETAILS SET CED_REC_VER=RECVER WHERE ID=MINID;
                        ELSE
                            SET RECVER=(SELECT MIN(CED_REC_VER) FROM TEMP_PAYMENT_CUSTOMER WHERE CED_REC_VER NOT IN(SELECT CED_REC_VER FROM TEMP_PAYMENT_ENTRY_DETAILS WHERE CUSTOMER_ID=MINCUSTOMERID AND PD_FOR_PERIOD=FORPERIOD AND PP_ID=2)AND CUSTOMER_ID=MINCUSTOMERID AND CED_STARTDATE=FORPERIOD AND UNIT_ID=UNITID);
                            UPDATE TEMP_PAYMENT_ENTRY_DETAILS SET CED_REC_VER=RECVER WHERE ID=MINID;
                        END IF;
                        IF RECVER IS NULL THEN
                            SET @SET_RECVER=(SELECT CONCAT('SELECT MIN(TCED.CED_REC_VER) INTO @REC_VER FROM ',DESTINATIONSCHEMA,'.TEMP_CUSTOMER_ENTRY_DETAILS TCED WHERE  ','"',FORPERIOD,'"',' BETWEEN (SELECT ADDDATE(LAST_DAY(SUBDATE(TCED.CED_STARTDATE, INTERVAL 1 MONTH)), 1)) AND IF(TCED.CED_PRETERMINATE_DATE IS NOT NULL,TCED.CED_PRETERMINATE_DATE,TCED.CED_ENDDATE) AND IF(TCED.CED_PRETERMINATE_DATE IS NOT NULL,TCED.CED_PRETERMINATE_DATE>TCED.CED_STARTDATE,TCED.CED_ENDDATE>TCED.CED_STARTDATE) AND TCED.UNIT_ID=',UNITID,' AND TCED.CUSTOMER_ID=',MINCUSTOMERID));
                            PREPARE SET_RECVER FROM @SET_RECVER;
                            EXECUTE SET_RECVER;
                            SET RECVER=@REC_VER;
							SET RECVER=@REC_VER;
							IF NOT EXISTS(SELECT CED_REC_VER FROM TEMP_PAYMENT_ENTRY_DETAILS WHERE CUSTOMER_ID=MINCUSTOMERID AND CED_REC_VER=RECVER AND PP_ID=2 AND PD_FOR_PERIOD=FORPERIOD)THEN
								SET RECVER=@REC_VER;
							ELSE
								SET @SET_RECVER=(SELECT CONCAT('SELECT MIN(TCED.CED_REC_VER) INTO @REC_VER FROM ',DESTINATIONSCHEMA,'.TEMP_CUSTOMER_ENTRY_DETAILS TCED WHERE  ','"',FORPERIOD,'"',' BETWEEN (SELECT ADDDATE(LAST_DAY(SUBDATE(TCED.CED_STARTDATE, INTERVAL 1 MONTH)), 1)) AND IF(TCED.CED_PRETERMINATE_DATE IS NOT NULL,TCED.CED_PRETERMINATE_DATE,TCED.CED_ENDDATE) AND IF(TCED.CED_PRETERMINATE_DATE IS NOT NULL,TCED.CED_PRETERMINATE_DATE>TCED.CED_STARTDATE,TCED.CED_ENDDATE>TCED.CED_STARTDATE) AND TCED.CED_REC_VER NOT IN(SELECT CED_REC_VER FROM TEMP_PAYMENT_ENTRY_DETAILS WHERE CUSTOMER_ID=',MINCUSTOMERID,' AND PD_FOR_PERIOD=','"',FORPERIOD,'"',' AND PP_ID=2) AND TCED.UNIT_ID=',UNITID,' AND TCED.CUSTOMER_ID=',MINCUSTOMERID));
								PREPARE SET_RECVER FROM @SET_RECVER;
								EXECUTE SET_RECVER; 
								SET RECVER=@REC_VER;
							END IF;
                            SET STARTDATE=(SELECT CED_STARTDATE FROM TEMP_PAYMENT_CUSTOMER WHERE CUSTOMER_ID=MINCUSTOMERID AND CED_REC_VER=RECVER);
                            UPDATE TEMP_PAYMENT_ENTRY_DETAILS SET CED_REC_VER=RECVER WHERE ID=MINID;
                            UPDATE TEMP_PAYMENT_ENTRY_DETAILS SET PD_FOR_PERIOD=STARTDATE WHERE ID=MINID;
                            SET @REC_VER=NULL;
                        END IF;
                    END IF;
                    IF (PPID=3) THEN
                        SET UNITID=(SELECT UNIT_ID FROM TEMP_PAYMENT_ENTRY_DETAILS WHERE ID=MINID);
                        SET RECVER=(SELECT MIN(CED_REC_VER) FROM TEMP_PAYMENT_CUSTOMER WHERE CUSTOMER_ID=MINCUSTOMERID AND CED_STARTDATE=FORPERIOD AND UNIT_ID=UNITID);
                        IF NOT EXISTS(SELECT CED_REC_VER FROM TEMP_PAYMENT_ENTRY_DETAILS WHERE CUSTOMER_ID=MINCUSTOMERID AND CED_REC_VER=RECVER AND PP_ID=3 AND PD_FOR_PERIOD=FORPERIOD)THEN
                            UPDATE TEMP_PAYMENT_ENTRY_DETAILS SET CED_REC_VER=RECVER WHERE ID=MINID;
                            ELSE
                            SET RECVER=(SELECT MIN(CED_REC_VER) FROM TEMP_PAYMENT_CUSTOMER WHERE CED_REC_VER NOT IN(SELECT CED_REC_VER FROM TEMP_PAYMENT_ENTRY_DETAILS WHERE CUSTOMER_ID=MINCUSTOMERID AND PD_FOR_PERIOD=FORPERIOD AND PP_ID=3)AND CUSTOMER_ID=MINCUSTOMERID AND CED_STARTDATE=FORPERIOD AND UNIT_ID=UNITID);
                            UPDATE TEMP_PAYMENT_ENTRY_DETAILS SET CED_REC_VER=RECVER WHERE ID=MINID;
                        END IF;
                        IF RECVER IS NULL THEN
                            SET @SET_RECVER=(SELECT CONCAT('SELECT MIN(TCED.CED_REC_VER) INTO @REC_VER FROM ',DESTINATIONSCHEMA,'.TEMP_CUSTOMER_ENTRY_DETAILS TCED WHERE  ','"',FORPERIOD,'"',' BETWEEN (SELECT ADDDATE(LAST_DAY(SUBDATE(TCED.CED_STARTDATE, INTERVAL 1 MONTH)), 1)) AND IF(TCED.CED_PRETERMINATE_DATE IS NOT NULL,TCED.CED_PRETERMINATE_DATE,TCED.CED_ENDDATE) AND IF(TCED.CED_PRETERMINATE_DATE IS NOT NULL,TCED.CED_PRETERMINATE_DATE>TCED.CED_STARTDATE,TCED.CED_ENDDATE>TCED.CED_STARTDATE)AND TCED.UNIT_ID=',UNITID,' AND TCED.CUSTOMER_ID=',MINCUSTOMERID));
							...
							IF NOT EXISTS(SELECT CED_REC_VER FROM TEMP_PAYMENT_ENTRY_DETAILS WHERE CUSTOMER_ID=MINCUSTOMERID AND CED_REC_VER=RECVER AND PP_ID=3 AND PD_FOR_PERIOD=FORPERIOD)THEN
								SET RECVER=@REC_VER;
							ELSE
								SET @SET_RECVER=(SELECT CONCAT('SELECT MIN(TCED.CED_REC_VER) INTO @REC_VER FROM ',DESTINATIONSCHEMA,'.TEMP_CUSTOMER_ENTRY_DETAILS TCED WHERE  ','"',FORPERIOD,'"',' BETWEEN (SELECT ADDDATE(LAST_DAY(SUBDATE(TCED.CED_STARTDATE, INTERVAL 1 MONTH)), 1)) AND IF(TCED.CED_PRETERMINATE_DATE IS NOT NULL,TCED.CED_PRETERMINATE_DATE,TCED.CED_ENDDATE) AND  IF(TCED.CED_PRETERMINATE_DATE IS NOT NULL,TCED.CED_PRETERMINATE_DATE>TCED.CED_STARTDATE,TCED.CED_ENDDATE>TCED.CED_STARTDATE) AND TCED.CED_REC_VER NOT IN(SELECT CED_REC_VER FROM TEMP_PAYMENT_ENTRY_DETAILS WHERE CUSTOMER_ID=',MINCUSTOMERID,' AND PD_FOR_PERIOD=','"',FORPERIOD,'"',' AND PP_ID=3) AND TCED.UNIT_ID=',UNITID,' AND TCED.CUSTOMER_ID=',MINCUSTOMERID));
								PREPARE SET_RECVER FROM @SET_RECVER;
								EXECUTE SET_RECVER; 
								SET RECVER=@REC_VER;
							END IF;
                    ...
                    END IF;
                    IF(PPID=5)THEN
                        UPDATE TEMP_PAYMENT_CUSTOMER SET CED_ENDDATE=(SELECT ADDDATE(LAST_DAY(SUBDATE(CED_ENDDATE, INTERVAL 1 MONTH)), 1));
						UPDATE TEMP_PAYMENT_CUSTOMER SET CED_PRETERMINATE_DATE=(SELECT ADDDATE(LAST_DAY(SUBDATE(CED_PRETERMINATE_DATE, INTERVAL 1 MONTH)), 1));
                        SET UNITID=(SELECT UNIT_ID FROM TEMP_PAYMENT_ENTRY_DETAILS WHERE ID=MINID);
                        SET RECVER=(SELECT MIN(CED_REC_VER) FROM TEMP_PAYMENT_CUSTOMER WHERE CUSTOMER_ID=MINCUSTOMERID AND IF (CED_PRETERMINATE_DATE IS NOT NULL,CED_PRETERMINATE_DATE=FORPERIOD,CED_ENDDATE=FORPERIOD) AND UNIT_ID=UNITID);
                        IF NOT EXISTS(SELECT CED_REC_VER FROM TEMP_PAYMENT_ENTRY_DETAILS WHERE CUSTOMER_ID=MINCUSTOMERID AND CED_REC_VER=RECVER AND PP_ID=5 AND PD_FOR_PERIOD=FORPERIOD)THEN
                            UPDATE TEMP_PAYMENT_ENTRY_DETAILS SET CED_REC_VER=RECVER WHERE ID=MINID;
                            ELSE
                            SET RECVER=(SELECT MIN(CED_REC_VER) FROM TEMP_PAYMENT_CUSTOMER WHERE CED_REC_VER NOT IN(SELECT CED_REC_VER FROM TEMP_PAYMENT_ENTRY_DETAILS WHERE CUSTOMER_ID=MINCUSTOMERID AND PD_FOR_PERIOD=FORPERIOD AND PP_ID=5)AND CUSTOMER_ID=MINCUSTOMERID AND CED_ENDDATE=FORPERIOD AND UNIT_ID=UNITID);
                            UPDATE TEMP_PAYMENT_ENTRY_DETAILS SET CED_REC_VER=RECVER WHERE ID=MINID;
                        END IF;
                        IF RECVER IS NULL THEN
                            SET @SET_RECVER=(SELECT CONCAT('SELECT MIN(TCED.CED_REC_VER) INTO @REC_VER FROM ',DESTINATIONSCHEMA,'.TEMP_CUSTOMER_ENTRY_DETAILS TCED WHERE  ','"',FORPERIOD,'"',' BETWEEN (SELECT ADDDATE(LAST_DAY(SUBDATE(TCED.CED_STARTDATE, INTERVAL 1 MONTH)), 1)) AND IF(TCED.CED_PRETERMINATE_DATE IS NOT NULL,TCED.CED_PRETERMINATE_DATE,TCED.CED_ENDDATE) AND  IF(TCED.CED_PRETERMINATE_DATE IS NOT NULL,TCED.CED_PRETERMINATE_DATE>TCED.CED_STARTDATE,TCED.CED_ENDDATE>TCED.CED_STARTDATE)AND TCED.UNIT_ID=',UNITID,' AND TCED.CUSTOMER_ID=',MINCUSTOMERID));
							...
							IF NOT EXISTS(SELECT CED_REC_VER FROM TEMP_PAYMENT_ENTRY_DETAILS WHERE CUSTOMER_ID=MINCUSTOMERID AND CED_REC_VER=RECVER AND PP_ID=5 AND PD_FOR_PERIOD=FORPERIOD)THEN
								SET RECVER=@REC_VER;
							ELSE
								SET @SET_RECVER=(SELECT CONCAT('SELECT MIN(TCED.CED_REC_VER) INTO @REC_VER FROM ',DESTINATIONSCHEMA,'.TEMP_CUSTOMER_ENTRY_DETAILS TCED WHERE  ','"',FORPERIOD,'"',' BETWEEN (SELECT ADDDATE(LAST_DAY(SUBDATE(TCED.CED_STARTDATE, INTERVAL 1 MONTH)), 1)) AND IF(TCED.CED_PRETERMINATE_DATE IS NOT NULL,TCED.CED_PRETERMINATE_DATE,TCED.CED_ENDDATE) AND IF(TCED.CED_PRETERMINATE_DATE IS NOT NULL,TCED.CED_PRETERMINATE_DATE>TCED.CED_STARTDATE,TCED.CED_ENDDATE>TCED.CED_STARTDATE) AND TCED.CED_REC_VER NOT IN(SELECT CED_REC_VER FROM TEMP_PAYMENT_ENTRY_DETAILS WHERE CUSTOMER_ID=',MINCUSTOMERID,' AND PD_FOR_PERIOD=','"',FORPERIOD,'"',' AND PP_ID=5) AND TCED.UNIT_ID=',UNITID,' AND TCED.CUSTOMER_ID=',MINCUSTOMERID));
								PREPARE SET_RECVER FROM @SET_RECVER;
								EXECUTE SET_RECVER; 
								SET RECVER=@REC_VER;
							END IF;
                            SET ENDDATE=(SELECT CED_ENDDATE FROM TEMP_PAYMENT_CUSTOMER WHERE CUSTOMER_ID=MINCUSTOMERID AND CED_REC_VER=RECVER);
							SET PRE_DATE=(SELECT CED_PRETERMINATE_DATE FROM TEMP_PAYMENT_CUSTOMER WHERE CUSTOMER_ID=MINCUSTOMERID AND CED_REC_VER=RECVER);
                            UPDATE TEMP_PAYMENT_ENTRY_DETAILS SET CED_REC_VER=RECVER WHERE ID=MINID;
							IF PRE_DATE IS NOT NULL THEN
                            UPDATE TEMP_PAYMENT_ENTRY_DETAILS SET PD_FOR_PERIOD=PRE_DATE WHERE ID=MINID;
							ELSE
							UPDATE TEMP_PAYMENT_ENTRY_DETAILS SET PD_FOR_PERIOD=ENDDATE WHERE ID=MINID;
							END IF;
                            SET @REC_VER=NULL;
                        END IF;
                    END IF;
...
CREATE TABLE TEMP_DURATION_PAYMENT_PART5(ID INTEGER PRIMARY KEY AUTO_INCREMENT,SPLITTING_DURATION5 TIME);
SET INITTIME='00:00:00';
SET MINDURATIONID=(SELECT MIN(ID) FROM TEMP_DURATION_PAYMENT_PART4);
SET MAXDURATIONID=(SELECT MAX(ID)FROM TEMP_DURATION_PAYMENT_PART4);
WHILE MINDURATIONID<=MAXDURATIONID DO
SET INITTIME=(SELECT ADDTIME(INITTIME,(SELECT SPLITTING_DURATION4 FROM TEMP_DURATION_PAYMENT_PART4 WHERE ID=MINDURATIONID)));
SET MINDURATIONID=MINDURATIONID+1;
END WHILE;
INSERT INTO TEMP_DURATION_PAYMENT_PART5(SPLITTING_DURATION5)VALUES(INITTIME);
COMMIT;
END;
