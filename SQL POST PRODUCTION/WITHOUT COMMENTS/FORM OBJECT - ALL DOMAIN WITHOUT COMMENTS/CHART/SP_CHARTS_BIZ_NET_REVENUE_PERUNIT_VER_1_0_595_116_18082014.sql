...
DECLARE T1_VALUE DECIMAL(10,2) DEFAULT 0;
DECLARE T2_VALUE DECIMAL(10,2) DEFAULT 0;
DECLARE LI_MONTH_YEAR_T2  VARCHAR(20) DEFAULT 0;
DECLARE LI_VALUE_T2       DECIMAL(10,2) DEFAULT 0;
DECLARE I INT DEFAULT 1;
DECLARE J INT DEFAULT 1;
DECLARE T2_COUNT INT DEFAULT 0;
DECLARE T3_COUNT INT DEFAULT 0;
DECLARE FVALUE DECIMAL(10,2) DEFAULT 0;
...
DECLARE INTERMEDIATE_CHARTS_BIZ_NET_REVENUE_PERUNIT TEXT;
DECLARE TEMP_INTERMEDIATE_BIZ_NET_REVENUE_PERUNIT TEXT;
DECLARE USERSTAMP_ID INT;
DECLARE EXIT HANDLER FOR SQLEXCEPTION
BEGIN
 ROLLBACK;
 IF(CHARTS_INTERMEDIATE_BIZ_EXPENSE_TMPTBL IS NOT NULL) THEN
...
END IF;	
IF(INTERMEDIATE_CHARTS_GROSS_REVENUE_TMPTBL IS NOT NULL) THEN
...
END IF;
IF(CHARTS_GROSS_REVENUE_PERUNIT_TMPTBL IS NOT NULL) THEN
...
END IF;
IF(CHARTS_BIZ_EXPENSE_PERUNIT_TMPTBL IS NOT NULL) THEN
...
END IF;
IF(TEMP_INTERMEDIATE_BIZ_NET_REVENUE_PERUNIT IS NOT NULL) THEN
    SET @DROP_TEMP_INTERMEDIATE_BIZ_NET_REVENUE_PERUNIT=(SELECT CONCAT('DROP TABLE IF EXISTS ',TEMP_INTERMEDIATE_BIZ_NET_REVENUE_PERUNIT));
		PREPARE DROP_TEMP_INTERMEDIATE_BIZ_NET_REVENUE_PERUNIT_STMT FROM @DROP_TEMP_INTERMEDIATE_BIZ_NET_REVENUE_PERUNIT;
		EXECUTE DROP_TEMP_INTERMEDIATE_BIZ_NET_REVENUE_PERUNIT_STMT;
    END IF;
...
START TRANSACTION;
  ...
  SET INTERMEDIATE_CHARTS_BIZ_NET_REVENUE_PERUNIT=(SELECT CONCAT('TEMP_INTERMEDIATE_BIZ_NET_REVENUE_PERUNIT',SYSDATE()));
	...
	SET INTERMEDIATE_CHARTS_BIZ_NET_REVENUE_PERUNIT=(SELECT REPLACE(INTERMEDIATE_CHARTS_BIZ_NET_REVENUE_PERUNIT,' ',''));
	SET INTERMEDIATE_CHARTS_BIZ_NET_REVENUE_PERUNIT=(SELECT REPLACE(INTERMEDIATE_CHARTS_BIZ_NET_REVENUE_PERUNIT,'-',''));
	SET INTERMEDIATE_CHARTS_BIZ_NET_REVENUE_PERUNIT=(SELECT REPLACE(INTERMEDIATE_CHARTS_BIZ_NET_REVENUE_PERUNIT,':',''));
	SET TEMP_INTERMEDIATE_BIZ_NET_REVENUE_PERUNIT=(SELECT CONCAT(INTERMEDIATE_CHARTS_BIZ_NET_REVENUE_PERUNIT,'_',USERSTAMP_ID));
		CALL SP_FORMAT_DATE(FROM_DATE,TO_DATE,@FD,@TD);
		SELECT @FD INTO FINAL_FROM_DATE;
		SELECT @TD INTO FINAL_TO_DATE;  
		SET @CREATE_TEMP_CHARTS_BIZ_EXPENSE_PERUNIT=(SELECT CONCAT('CREATE TABLE ',CHARTS_BIZ_EXPENSE_PERUNIT_TMPTBL,'(ID INT NOT NULL AUTO_INCREMENT,MONTH_YEAR VARCHAR(20),TOTAL_BIZ_EXP DECIMAL(10,2),SORT_DATE DATE, PRIMARY KEY (ID))'));
		...
		SET @CREATE_TEMP_INTERMEDIATE_CHARTS_BIZ=(SELECT CONCAT('CREATE TABLE ',CHARTS_INTERMEDIATE_BIZ_EXPENSE_TMPTBL,'(MONTH_YEAR VARCHAR(20),CAR_PARK DECIMAL(10,2),DIGITAL_VOICE DECIMAL(10,2),ELECTRICITY DECIMAL(10,2),FACILITY_USE DECIMAL(10,2),MOVE_IN_OUT DECIMAL(10,2),STARHUB DECIMAL(10,2),UNIT_EXPENSE DECIMAL(10,2), DATE DATE)'));
		...
		SET @CREATE_TEMP_CHARTS_GROSS_REVENUE_PERUNIT=(SELECT CONCAT('CREATE TABLE ',CHARTS_GROSS_REVENUE_PERUNIT_TMPTBL,'(ID INT NOT NULL AUTO_INCREMENT, MONTH_YEAR VARCHAR(20),GROSS_REVENUE DECIMAL(10,2),SORT_DATE DATE, PRIMARY KEY (ID))'));
		...
		SET @CREATE_TEMP_INTERMEDIATE_CHARTS_GROSS_REVENUE=(SELECT CONCAT('CREATE TABLE ',INTERMEDIATE_CHARTS_GROSS_REVENUE_TMPTBL,'(MONTH_YEAR VARCHAR(20),GROSS_REVENUE DECIMAL(10,2),  DATE DATE)'));
...
SET @CREATE_VIEW_BIZ_NET_REVENUE =(SELECT CONCAT('CREATE OR REPLACE VIEW VIEW_BIZ_NET_REVENUE AS SELECT T1.MONTH_YEAR, (T1.GROSS_REVENUE-IFNULL(T2.TOTAL_BIZ_EXP,0)) AS REVENUE, T1.SORT_DATE FROM ',CHARTS_GROSS_REVENUE_PERUNIT_TMPTBL,' T1 LEFT JOIN ',CHARTS_BIZ_EXPENSE_PERUNIT_TMPTBL,' T2 ON T1.MONTH_YEAR = T2.MONTH_YEAR UNION SELECT T2.MONTH_YEAR, (IFNULL(T1.GROSS_REVENUE,0)-T2.TOTAL_BIZ_EXP) AS REVENUE, T2.SORT_DATE FROM ',CHARTS_GROSS_REVENUE_PERUNIT_TMPTBL,' T1 RIGHT JOIN ',CHARTS_BIZ_EXPENSE_PERUNIT_TMPTBL,' T2 ON T1.MONTH_YEAR = T2.MONTH_YEAR'));
		...
		SET @CREATE_TEMP_INTERMEDIATE_BIZ_NET_REVENUE_PERUNIT=(SELECT CONCAT('CREATE TABLE ',TEMP_INTERMEDIATE_BIZ_NET_REVENUE_PERUNIT,'(MONTHYEAR VARCHAR(20), GROSSREVENUE DECIMAL(10,2), TOTALRENTAL_BIZ DECIMAL(10,2), NETREVENUE DECIMAL(10,2), UNITRENTAL DECIMAL(10,2), TOTALBIZ DECIMAL(10,2),SORTDATE DATE)'));
		PREPARE CREATE_TEMP_INTERMEDIATE_BIZ_NET_REVENUE_PERUNIT_STMT FROM @CREATE_TEMP_INTERMEDIATE_BIZ_NET_REVENUE_PERUNIT;
		EXECUTE CREATE_TEMP_INTERMEDIATE_BIZ_NET_REVENUE_PERUNIT_STMT;    
		SET @INSERT_TEMP_INTERMEDIATE_BIZ_NET_REVENUE_PERUNIT=(SELECT CONCAT('INSERT INTO ',TEMP_INTERMEDIATE_BIZ_NET_REVENUE_PERUNIT,' (MONTHYEAR,NETREVENUE,SORTDATE,UNITRENTAL)SELECT A.MONTH_YEAR,A.REVENUE,A.SORT_DATE,D.UD_PAYMENT FROM VIEW_BIZ_NET_REVENUE A,UNIT_DETAILS D INNER JOIN UNIT F ON D.UNIT_ID = F.UNIT_ID AND F.UNIT_NO =',UNIT_NO));
		PREPARE INSERT_TEMP_INTERMEDIATE_BIZ_NET_REVENUE_PERUNIT_STMT FROM @INSERT_TEMP_INTERMEDIATE_BIZ_NET_REVENUE_PERUNIT;
		EXECUTE INSERT_TEMP_INTERMEDIATE_BIZ_NET_REVENUE_PERUNIT_STMT;   
		SET @INSERT_TEMP_INTERMEDIATE_BIZ_NET_REVENUE_PERUNIT=(SELECT CONCAT('INSERT INTO ',TEMP_INTERMEDIATE_BIZ_NET_REVENUE_PERUNIT,' (MONTHYEAR,GROSSREVENUE,SORTDATE) SELECT MONTH_YEAR,GROSS_REVENUE,SORT_DATE FROM ',CHARTS_GROSS_REVENUE_PERUNIT_TMPTBL));
		PREPARE INSERT_TEMP_INTERMEDIATE_BIZ_NET_REVENUE_PERUNIT_STMT FROM @INSERT_TEMP_INTERMEDIATE_BIZ_NET_REVENUE_PERUNIT;
		EXECUTE INSERT_TEMP_INTERMEDIATE_BIZ_NET_REVENUE_PERUNIT_STMT;    
        SET @INSERT_TEMP_INTERMEDIATE_BIZ_NET_REVENUE_PERUNIT=(SELECT CONCAT('INSERT INTO ',TEMP_INTERMEDIATE_BIZ_NET_REVENUE_PERUNIT,' (MONTHYEAR,TOTALRENTAL_BIZ,SORTDATE)SELECT MONTH_YEAR,TOTAL_BIZ_EXP,SORT_DATE FROM ',CHARTS_BIZ_EXPENSE_PERUNIT_TMPTBL));
		PREPARE INSERT_TEMP_INTERMEDIATE_BIZ_NET_REVENUE_PERUNIT_STMT FROM @INSERT_TEMP_INTERMEDIATE_BIZ_NET_REVENUE_PERUNIT;
		EXECUTE INSERT_TEMP_INTERMEDIATE_BIZ_NET_REVENUE_PERUNIT_STMT;    
		SET @INSERT_TEMP_INTERMEDIATE_BIZ_NET_REVENUE_PERUNIT=(SELECT CONCAT('INSERT INTO ',TEMP_INTERMEDIATE_BIZ_NET_REVENUE_PERUNIT,' (MONTHYEAR,TOTALBIZ,SORTDATE) SELECT MONTH_YEAR,AMOUNT,DATE FROM VIEW_BIZ_EXPENSE'));
		PREPARE INSERT_TEMP_INTERMEDIATE_BIZ_NET_REVENUE_PERUNIT_STMT FROM @INSERT_TEMP_INTERMEDIATE_BIZ_NET_REVENUE_PERUNIT;
		EXECUTE INSERT_TEMP_INTERMEDIATE_BIZ_NET_REVENUE_PERUNIT_STMT;    
		SET @CREATE_TEMP_CHARTS_BIZ_NET_REVENUE_PERUNIT=(SELECT CONCAT('CREATE TABLE ',CHARTS_BIZ_NET_REVENUE_PERUNIT_TMPTBL,'(MONTH_YEAR VARCHAR(20), GROSS_REVENUE DECIMAL(10,2), TOTAL_RENTAL_BIZ DECIMAL(10,2), REVENUE DECIMAL(10,2), UNIT_RENTAL DECIMAL(10,2), TOTAL_BIZ DECIMAL(10,2))'));
		...
		SET @INSERT_TEMP_CHARTS_BIZ_NET_REVENUE_PERUNIT=(SELECT CONCAT('INSERT INTO ',CHARTS_BIZ_NET_REVENUE_PERUNIT_TMPTBL,' (MONTH_YEAR, GROSS_REVENUE, TOTAL_RENTAL_BIZ, REVENUE, UNIT_RENTAL, TOTAL_BIZ)SELECT MONTHYEAR,IFNULL(SUM(GROSSREVENUE),0),IFNULL(SUM(TOTALRENTAL_BIZ),0),IFNULL(SUM(NETREVENUE),0),IFNULL(SUM(UNITRENTAL),0),IFNULL(SUM(TOTALBIZ),0) FROM ',TEMP_INTERMEDIATE_BIZ_NET_REVENUE_PERUNIT,' GROUP BY MONTHYEAR ORDER BY SORTDATE ASC'));
