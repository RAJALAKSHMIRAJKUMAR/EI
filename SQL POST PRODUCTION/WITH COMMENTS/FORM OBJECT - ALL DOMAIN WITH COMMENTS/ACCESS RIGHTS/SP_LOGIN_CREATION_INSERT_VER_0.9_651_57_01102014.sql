-- VERSION 0.9 STARTDATE:25/09/2014 ENDDATE:25/09/2014 ISSUE NO:651 COMMENTNO :57, DESC:REMOVED COMMIT,REMOVED DROP TEMP TABLE,TEMP TABLE GIVEN AS OUT PARAMETER BY PUNI
...
-- version 0.6 startdate:26/05/2014 --enddate:26/05/2014--- issueno 659 commentno:85 -->desc: CHANGED THE SP FOR INSERTION IF EMAIL PROFILE ID IS NOT IN THAT EMAIL LIST TABLE DONE BY:BHAVANI.R
...
CREATE PROCEDURE SP_LOGIN_CREATION_INSERT(LOGINID VARCHAR(40),
ROLENAME VARCHAR(15),
JOINDATE DATE,
USERSTAMP VARCHAR(50),
OUT TEMPTABLE_OUT_MENU TEXT,
OUT UR_FLAG INTEGER)
BEGIN
...
BEGIN 
	ROLLBACK; 
	SET UR_FLAG=0;
	IF (TEMPTABLE_MENU IS NOT NULL ) THEN
...
END IF;
  ...
  EXECUTE SELECT_MAXID_STMT;
    ...
    IF NOT EXISTS(SELECT EL_EMAIL_ID FROM EMAIL_LIST WHERE EP_ID=EPID AND EL_EMAIL_ID=LOGINID)THEN
