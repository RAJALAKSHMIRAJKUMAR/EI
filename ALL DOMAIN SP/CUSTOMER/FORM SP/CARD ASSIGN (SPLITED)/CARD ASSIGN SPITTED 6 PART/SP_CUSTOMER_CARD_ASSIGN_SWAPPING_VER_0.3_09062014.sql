...
IF (OLD_ACCESS_CARD_NO IS NOT NULL AND NEW_ACCESS_CARD_NO IS NOT NULL AND NEW_ACCESS_CARD_NO!=OLD_ACCESS_CARD_NO)THEN            
			 IF NOT EXISTS(SELECT * FROM CUSTOMER_LP_DETAILS WHERE CUSTOMER_ID=CA_CUSTOMER_ID AND CED_REC_VER=CA_REC_VER AND UASD_ID=(SELECT UASD_ID FROM UNIT_ACCESS_STAMP_DETAILS WHERE UASD_ACCESS_CARD=NEW_ACCESS_CARD_NO))THEN
            ...
            END IF;		    
              ...
              IF ((ACTIVE_RECVER IS NULL OR ACTIVE_RECVER IS NOT NULL )AND TERMINATE_RECVER IS NOT NULL) THEN 
                ...
                WHILE MINTERMID<=MAXTERMID DO
				  ...
				  SET @TICK_OLD_VALUE=(SELECT CONCAT('CED_REC_VER=',@TEMP_RECVER,',',
                  'UASD_ID=',@TEMP_UASDID));                  
                  ...
                  EXECUTE UPDATELPDETAILS_STMT;
				   ...
				   SET PRETERMINATE_DATE=@TEMPPRETERMINATE_DATE;
                    IF PRETERMINATE_DATE IS NULL THEN
                      SET @TICK_ACCESS_OLD_VALUE=(SELECT CONCAT('UASD_ID=',(SELECT UASD_ID FROM UNIT_ACCESS_STAMP_DETAILS WHERE UASD_ACCESS_CARD=OLD_ACCESS_CARD_NO),',','ACN_ID=NULL,CACD_VALID_TILL=NULL'));
					 ...
					 SET @UPDATE_CACD=(SELECT CONCAT('UPDATE CUSTOMER_ACCESS_CARD_DETAILS SET ACN_ID=(SELECT ACN_ID FROM ACCESS_CONFIGURATION WHERE ACN_DATA="TERMINATED"),CACD_VALID_TILL=@CACD_VALIDTILL,ULD_ID=',USERSTAMP_ID,'  WHERE CACD_ID=(SELECT CACD_ID FROM ',TEMP_CUSTOMER_ACCESS_CARD,' WHERE UASD_ID=(SELECT UASD_ID FROM UNIT_ACCESS_STAMP_DETAILS WHERE UASD_ACCESS_CARD=',OLD_ACCESS_CARD_NO,'))'));
                ...
                SET @TICK_ACCESS_OLD_VALUE=(SELECT CONCAT('UASD_ID=',ACCESSOLDCARD));
