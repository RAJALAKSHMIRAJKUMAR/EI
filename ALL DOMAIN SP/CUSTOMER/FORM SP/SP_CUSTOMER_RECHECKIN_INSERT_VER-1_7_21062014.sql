...
--  variable for CUSTOMER_FEE_DETAILS table
...
--  variable for CUSTOMER_ACCESS_CARD_DETAILS table
IN RE_CARD_NO TEXT,
IN RE_VALID_FROM DATE,
IN USERSTAMP VARCHAR(50),

--  variable for CUSTOMER_LP_DETAILS table
IN RE_STARTDATE DATE,
IN RE_ENDDATE DATE,
IN RE_GUEST_CARD TEXT,

--  variable for CUSTOMER_PERSONAL_DETAILS table
...
-- parameters for customer_company_details table
...
-- parameters for CUSTOMER_PERSONAL_DETAILS table
...
-- parameters for CUSTOMER_PERSONAL_DETAILS table
	...
	DECLARE CARDNO TEXT;
	DECLARE CUSTOMER_RECHECKIN_TEMPTBLNAME TEXT;
...
--  set null for nan madotary fields CUSTOMER_ENTRY_DETAILS table
...
--  set null for nan madotary fields CUSTOMER_ACCESS_CARD_DETAILS table 
	IF RE_CARD_NO='' THEN
		SET RE_CARD_NO=NULL;
	END IF;

--  set null for nan madotary fields CUSTOMER_LP_DETAILS table 
	IF RE_GUEST_CARD='' THEN
		SET RE_GUEST_CARD=NULL;
	END IF;

--  set null for nan madotary fields CUSTOMER_PERSONAL_DETAILS table 
	IF RE_MOBILE='' THEN
		SET RE_MOBILE=NULL;
	END IF;
...
--  CREATING TEMP TABLE 	
	IF(RE_CARD_NO IS NOT NULL) THEN
		...
		--  TEMP TABLE NAME END
-- temp table create query for TEMP_CREATE_RE_CARD_NO
...
--  QUERY FOR THROWING MESSAGE IF CARD IS ACTIVE OR CARD IS LOST	
		...
		SET CUSTOMER_RECHECKIN_SUCCESS_FLAG = (SELECT CONCAT(CUSTOMER_RECHECKIN_SUCCESS_FLAG, ' - ACTIVE CARD(S)SHLD NOT BE ASSIGNED'));
...
--  INSERT QUERY FOR CUSTOMER_COMPANY_DETAILS (CUSTOMER_ID NOT EXISTS)
		...
		IF (RE_COMPANY_NAME IS NULL AND RE_COMPANY_ADDR IS NULL AND RE_POSTAL_CODE IS NULL AND RE_OFFICE_NO IS NULL) THEN
			...
			IF EXISTS(SELECT CUSTOMER_ID FROM CUSTOMER_ENTRY_DETAILS WHERE CUSTOMER_ID=RE_CUSTOMER_ID) THEN
				...
				UPDATE TICKLER_HISTORY SET CUSTOMER_ID=RE_CUSTOMER_ID WHERE TH_ID=THID;
...
--  UPDATE QUERY FOR CUSTOMER_PERSONAL_DETAILS (CUSTOMER_ID EXISTS)
