...
IF BASIC_ROLES IS NULL THEN
  ...
  DO_THIS_DELETE:
  LOOP
    SET V_STRLEN1 = CHAR_LENGTH(BASIC_ROLES1); -- getting the character lengh

    SET V_BASIC_ROLE1 = SUBSTRING_INDEX(BASIC_ROLES1, ',', 1); -- splitting the value
...
DO_THIS_FIRST:
  LOOP
    SET V_STRLEN2 = CHAR_LENGTH(BASIC_ROLES); -- getting the character lengh

    SET V_BASIC_ROLE2 = SUBSTRING_INDEX(BASIC_ROLES, ',', 1); -- splitting the value
    ...
    SELECT COUNT(BRP_ID) INTO V_COUNT_1 FROM BASIC_ROLE_PROFILE WHERE BRP_BR_ID = V_BASIC_ROLES_ID2 AND URC_ID = V_URC_ID;
     IF (V_COUNT_1 = 0) THEN
     -- inserting each roles in the table
  ...
  END IF;
   ...
   -- deleting menus which is not available in the input during updation     
