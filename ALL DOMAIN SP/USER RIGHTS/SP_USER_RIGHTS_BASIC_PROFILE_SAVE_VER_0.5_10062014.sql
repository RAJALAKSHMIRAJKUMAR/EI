--version 0.5 startdate:10/06/2014 --enddate:10/06/2014--- issueno 566 commentno:12-->desc: ADDED ROLLBACK AND COMMIT-SASIKALA
--version 0.4 startdate:28/02/2014 --enddate:28/02/2014--- issueno 754 commentno:36-->desc: APPLIED SUB_SP TO REPLACE USERSTAMP AS ID-SAFI
--version 0.3 --sdate:27/02/2014 --edate:27/02/2014 --issue:754 --comment:22 --doneby:RL	
-- VER 0.2 ISSUE NO:659 COMMENT #39 STARTDATE:06/01/2014 ENDDATE:08/01/2014 DESC:SP FOR USER RIGHTS BASIC PROFILE SAVE. DONE BY:MANIKANDAN.S
...
DO_THIS_FIRST:
  LOOP
    SET V_STRLEN = CHAR_LENGTH(BASIC_ROLES); -- getting the character lengh

    SET V_BASIC_ROLE = SUBSTRING_INDEX(BASIC_ROLES, ',', 1); -- splitting the value
