-- VER 0.4 ISSUE NO:817 COMMENT #139 STARTDATE:21/06/2014 ENDDATE:21/06/2014 DESC:DROPPING TEMP TABLE IF ROLLBACK OCCURS DONE BY:SASIKALA
-- VER 0.3 ISSUE NO:566 COMMENT #12 STARTDATE:09/06/2014 ENDDATE:09/06/2014 DESC:ADDED ROLLBACK AND COMMIT DONE BY:SASIKALA
-- VER 0.2 ISSUE NO:817 COMMENT #35 STARTDATE:06/05/2014 ENDDATE:06/05/2014 DESC:CHANGING TEMP TABLE FOR DYNAMIC PURPOSE DONE BY:BHAVANI.R
-- VER 0.1 ISSUE NO:595 COMMENT #34 STARTDATE:10/01/2014 ENDDATE:10/01/2014 DESC:SP FOR CHARTS BIZ EXPENSE FOR PER UNIT. DONE BY:MANIKANDAN.S
...
--   TEMP TABLE NAME END    
  ...
  -- CREATING INTERMEDIATE TEMP TABLE 
  	SET @CREATE_TEMP_INTERMEDIATE_CHARTS_BIZ=(SELECT CONCAT('CREATE TABLE ',INTERMEDIATE_CHARTS_BIZ_EXPENSE_PERUNIT_TEMPTBL,'(MONTH_YEAR VARCHAR(20),CAR_PARK DECIMAL(7,2),DIGITAL_VOICE DECIMAL(7,2),ELECTRICITY DECIMAL(7,2),FACILITY_USE DECIMAL(7,2),MOVE_IN_OUT DECIMAL(7,2),STARHUB DECIMAL(7,2),UNIT_EXPENSE DECIMAL(7,2), DATE DATE)'));
...
END;
/* 
  CALL SP_CHARTS_BIZ_EXPENSE_PERUNIT(422, 'November-2013' , 'January-2014','EXPATSINTEGRATED@GMAIL.COM',@MAIN_TEMPTBL_CHARTS_BIZ_EXPENSE_PERUNIT);
  SELECT @MAIN_TEMPTBL_CHARTS_BIZ_EXPENSE_PERUNIT;
*/